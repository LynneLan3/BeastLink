...
test('formal routes UI: guides index + dynamic route hubs render from runtime routes', { timeout: 240_000 }, () => {
	...
	try {
		...
		assert.equal(
			(routesIndexHtml.match(/exp-task-route__visual/g) ?? []).length,
			2,
			'routes index should render the two visual routes from runtime config',
		);
		for (const routeCard of routesIndexHtml.match(/<a class="exp-task-route exp-task-route--content-only"[\s\S]*?<\/a>/g) ?? []) {
			assert.doesNotMatch(routeCard, /exp-task-route__visual/, 'content-only task routes must not render an empty visual frame');
		}
		// Task Map does NOT expand guide lists
