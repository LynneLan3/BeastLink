...
test('formal homepage: hub URL uses Experience Homepage with full data', { timeout: 240_000 }, () => {
	...
	try {
		...
		// Choose Your Route renders all 4 runtime routes with derived numbers.
		const routesHtml = html.match(/<div class="exp-routes">[\s\S]*?<\/div>\s*<\/div>/)?.[0] ?? '';
		...
		assert.match(routesHtml, /exp-number--watermark">01</);
		assert.match(routesHtml, /exp-route-entry__number">\s*04\s*<\/span>/);
