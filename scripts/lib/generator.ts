...
function yamlQuote(value: string): string {
	const yamlScalarKeywords = /^(?:true|false|null|~)$/i;
	const yamlDateLike = /^\d{4}-\d{2}-\d{2}$/;
	if (
		/^[A-Za-z0-9 _./:-]+$/.test(value) &&
		!/^[-?]/.test(value) &&
		!yamlScalarKeywords.test(value) &&
		!yamlDateLike.test(value) &&
		value !== ''
	) {
		return value;
	}
...
function assertNoCollision(
	...
	if (expectedManaged.has(relativePath) && existingManifest === null) {
		...
		if (/^public\/[0-9a-f-]{36}\.txt$/i.test(relativePath)) return;
	}
