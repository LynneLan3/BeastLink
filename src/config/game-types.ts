...
export interface HubPortalQuestion {
	label: string;
	href: string;
...
export interface HubPortalConfig {
	...
	popularQuestions?: readonly HubPortalQuestion[];
...
export interface GamePageConfig {
	id: string;
	/** Public slug path segment(s) after hub, e.g. `sample-game/overview`. */
	slug: string;
	role: PageRole;
...
export interface GameConfig {
	 ...
	 * Supported: `/` or a single segment like `/sample-game/`.
	 * Multi-segment hubs are rejected by validate:site.
	 */
	hubPath: string;
	/** Visible Hub H1. Falls back to `${name} Guide & Wiki` when omitted. */
	hubTitle?: string;
	...
	/** Optional Hub portal presentation. The generated homepage reads this; do not fork it per game. */
	portal?: HubPortalConfig;
